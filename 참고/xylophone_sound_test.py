...
def generate_xylophone_dingdongdaengdong():
    """정답 효과음: 실로폰 같은 '딩동댕동' 소리"""
    ...
    # 실로폰 음계: 딩(C) - 동(D) - 댕(E) - 동(D)
    notes = [
        (523.25, 0.0, 0.4),    # 딩 (C5)
        (587.33, 0.3, 0.4),    # 동 (D5)
        (659.25, 0.6, 0.4),    # 댕 (E5)
        (587.33, 0.9, 0.6)     # 동 (D5) - 마지막이라 좀 더 길게
    ]
...
def generate_error_sound():
    ...
    # 여러 불협화음 주파수들
    error_freqs = [220, 233, 185, 267]  # 서로 어울리지 않는 주파수들
    ...
    for i, freq in enumerate(error_freqs):
        ...
        if start_sample < total_samples:
            end_sample = min(start_sample + int(0.8 * sample_rate), total_samples)
            ...
            if wave_samples > 0:
                ...
                # 사각파로 거친 소리 생성
                square_wave = np.sign(np.sin(2 * np.pi * freq * t_wave)) * 0.2
                
                # 주파수 변조로 더 불안정한 소리
                modulation = 1 + 0.3 * np.sin(2 * np.pi * 7 * t_wave)
                square_wave *= modulation
                
                # 감쇠
                decay = np.exp(-t_wave * 2)
                square_wave *= decay
    ...
    noise_envelope = np.exp(-t * 3)  # 노이즈도 감쇠
    samples += noise * noise_envelope
    
    # 클리핑 방지
    samples = np.clip(samples, -0.8, 0.8)
...
def generate_xylophone_bright():
    ...
    # 더 높은 옥타브로 밝게: 딩(C6) - 동(D6) - 댕(E6) - 동(D6)
    notes = [
        (1046.5, 0.0, 0.35),   # 딩 (C6)
        (1174.7, 0.25, 0.35),  # 동 (D6)
        (1318.5, 0.5, 0.35),   # 댕 (E6)
        (1174.7, 0.75, 0.5)    # 동 (D6)
    ]
...
def generate_buzzer_error():
    """오답 효과음 버전2: 클래식 부저 오류음"""
    pygame.mixer.init(frequency=22050, size=-16, channels=2, buffer=512)
    ...
    # 페이드 아웃
    fade_start = int(0.6 * sample_rate)
...
def main():
    ...
    try:
        ...
        xylophone1 = generate_xylophone_dingdongdaengdong()
        print("✅ 정답 효과음 1: 실로폰 '딩동댕동' 생성 완료")
        
        xylophone2 = generate_xylophone_bright()
        print("✅ 정답 효과음 2: 밝은 실로폰 '딩동댕동' 생성 완료")
        
        error1 = generate_error_sound()
        print("✅ 오답 효과음 1: 시스템 오류음 생성 완료")
        
        error2 = generate_buzzer_error()
        print("✅ 오답 효과음 2: 클래식 부저음 생성 완료")
        ...
        sounds = [
            (xylophone1, "🎼 정답: 실로폰 '딩동댕동' (표준)"),
            (xylophone2, "✨ 정답: 실로폰 '딩동댕동' (밝은버전)"),
            (error1, "💥 오답: 시스템 오류음 (불협화음)"),
            (error2, "🚨 오답: 클래식 부저음")
        ]
        ...
        print("\n🎵 모든 실로폰 효과음 테스트 완료!")
        print("=" * 50)
        print("실로폰 '딩동댕동'과 오류음 중 어떤 조합이 마음에 드시나요?")
        print("\n💡 추천:")
        print("- 1라운드: 실로폰 표준버전 + 클래식 부저음")
        print("- 2라운드: 실로폰 밝은버전 + 시스템 오류음")
        print("\n🎼 실로폰 음계: C-D-E-D (도-레-미-레)")
